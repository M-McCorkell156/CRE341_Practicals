...
public class AI_FSM : MonoBehaviour
{
    ...
    [SerializeField] private Animator AIState_Piss; // child animator for the AI FSM
    ...
    private const string FSM_Pissing = "isPissing";
    ...
    void Update()
    {
        ...
        else if (fsm_anim.GetCurrentAnimatorStateInfo(0).IsName("Pissing"))
        {
            Debug.Log("Piss State");
            AIState_Patrol.enabled = false;
            AIState_Chase.enabled = false;
            AIState_Attack.enabled = false;
            AIState_Piss.enabled = true;
        }
    }
    ...
    void TopLevelFSMProcessing()
    {
        ...
        if (playerVisible)
        {
            ...
            int random = Random.Range(0, 499);
            //Debug.Log($"Dice roll for piss an got {random} ");
            if (random == 0)
            {
                Debug.Log("Sucess,they shall piss");
                fsm_anim.SetBool(FSM_Pissing, true);
            }
        }
        else
        {
